...
var vm = new Vue({
    ...
    methods: {
        load: function () {
            ...
            tale.get({
                url: '/cc/article/get/' + cid,
                success: function (data) {
                    $vm.article = data.data;
                    $vm.article.selected = [];

                    var selected = data.data.categories.split(',');
                    for(item in selected){
                        $vm.article.selected.push(selected[item]);
                    }

                    if ($vm.article.fmtType === 'markdown') {
                        mditor.value = data.data.content;
                    } else {
                        htmlEditor.summernote("code", data.data.content);
                    }
                    $vm.article.createdTime = moment($vm.article.created).format('YYYY-MM-DD HH:mm')
        ...
        autoSave: function (callback) {
            ...
            if ($vm.article.title !== '' && content !== '') {
                ...
                params.created = moment($('#form_datetime').val(), "YYYY-MM-DD HH:mm").unix()*1000;
                params.tags = $('#tags').val();

                tale.post({
                    url: '/cc/content/update',
                    data: params,
                    success: function (result) {
        ...
        publish: function (status) {
            ...
            $vm.autoSave(function () {
                tale.alertOk({
                    ...
                    then: function () {
                        setTimeout(function () {
                            window.location.href = '/cc/articles';
                        }, 500);
                    }
                });
            });
